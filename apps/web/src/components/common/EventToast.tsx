...
export default function EventToast() {
  ...
  return (
    <div className="fixed bottom-4 end-4 z-[9999] max-w-sm space-y-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`animate-in flex items-center gap-3 rounded-lg border p-3 shadow-lg ${TYPE_STYLES[toast.type] || TYPE_STYLES.info}`}
        >
